...
# ╔═╡ 130d4f3b-3163-414d-bb4c-672ebfa5f355
1 + 1

# ╔═╡ 2b6cb27d-7b42-4175-85eb-ec62bda48bc0
md"""
This is some [Markdown](https://www.markdownguide.org) text. ... with some math: ``e^{i \pi} + 1 = 0`` and a display: 
```math
   \int_0^\infty e^{-x} \, dx = 1
```

Or code with syntax highlighting:
```julia
s = 0
for i = 1:10
   s += i^2
...
**Proof:** we will prove this result as part of the first assignment. For now we will just show a simple numerical test that confirms it!
"""
...
begin 
	"interpolation nodes"
	xgrid(N) = [ π * j / N   for j = 0:(2N-1) ]
	
	"fourier coefficient indices"
	kgrid(N::Integer) = [ 0:N ; (-N+1):(-1) ]
end

# ╔═╡ 8d397e31-cb27-4d49-919c-4426acafc86f
# TODO - explain difference between 
#    begin - end and let blocks
...
# we use a begin-end block since the functions we define 
# here will be used elsewhere 
begin  
	...
	coeffs(t::TrigPoly) = t.F̂
	
	degree(t::TrigPoly) = length(coeffs(t)) ÷ 2

	kgrid(t::TrigPoly) = kgrid(degree(t))
	...
	function triginterp(f, N)
		...
		# system matrix
		A = [ exp(im * k * xj)  for k in kgrid(N), xj in X ]

		# coefficients are given by F̂ = A' * F as discussed above!
		return TrigPoly((A' * F) / (2*N))
	end 
	...
	# """
	# function evaltrig(x, F̂) 
	# 	N = length(F̂) ÷ 2
	# 	sum( real( F̂k * exp(im * k * x) )  
	# 		  for (F̂k, k) in zip(F̂, kgrid(N)) )
	# end
		

	(t::TrigPoly)(x) = sum(  real( F̂k * exp(im * k * x) )  
			                 for (F̂k, k) in zip( coeffs(t), kgrid(t) ) )
...
let N = 3
	...
	A = [ exp(im * k * xj)  for k in kgrid(N), xj in xgrid(N) ]
...
# ╔═╡ 2e37e16c-4b4e-4cb2-b0a3-0f4d5d838902
let f = x -> 1 / (1 + 3 * sin(x)^2), N = 4 
	t = triginterp(f, N)
	P = plot(f, -pi, pi, size = (400, 200))
	plot!(x -> t(x), -pi, pi)
end
...
# TODO: explain a bit how the slider and variable binding work...
md"""
We can just visually play around a bit with different target functions and degrees.....

Choose the function: $( @bind _fidx Slider(1:8, show_value=true) ) 

Choose the degree: $( @bind _N Slider(3:50, show_value=true) )
"""
...
let f = allf[_fidx], N = _N   # play with the functions and with the parameters! 
	...
	plot!(xp, t.(xp), lw=3, label = L"I_{%$N} f")
	plot!(X, f.(X), lw=0, m=:o, ms=4, c = 2, label = "", legend = :outertopright)
...
function triginterperror(f, N; Nerr = 1_362)
	# get the trigonometric interpolant coefficients 
	t = triginterp(f, N)
	# compute error on this grid
	xerr = range(0, 2π, length=Nerr)
	# compute the max-error on xerr grid and return it 
	return norm( f.(xerr) - t.(xerr), Inf)
end

# ╔═╡ 27242249-4cbb-4e39-a4a3-6828ee32dc94
begin
    x = 1.2345
	typeof(x)
	eps(Float64)
...
let NN = [1:2:5; (2).^(3:7)], flist = allf, flabels = flabels
	...
	P = plot( xaxis  = (L"N~{\rm (degree)}", ),
			  ...
			  size = (500, 300))
	# loop over all functions we want to test 
	for (f, lab) in zip(flist, flabels)
		# compute all errors for this f
		err = triginterperror.(f, NN)
		
		# add the error graph to the plot P 
...
P123 = let NN = 4:4:40, flist = allf[5:8], flabels = flabels[5:8]
...
# ╔═╡ dbb586ed-fb22-48ea-849b-342115d9dbc9
Plots.savefig(P123, "tmp.pdf")
